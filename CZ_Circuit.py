...
##Step 2: Add identities proportional to the amount of gates b/t c and t
		for a in range(2**(c-1)):			
			for d in range(2**(t-c-1)):
				j=2**(t-c-1)*(1+2*a)+d					
				circ_mat2[j][j] = 1
##Step 3: Add Z Gates at the target
		for a in range(2**(c-1)):
		...
		for a in range(2**(c-1)):	
			for b in range(2**(t-c-1)):			
				for d in range(2**(w-t)):
					...
					k=2*(2**(t-c-1)*(1+2*a)+b)		
					l=2*(2**(t-c-1)*(1+2*a)+b)+1	
					circ_mat4[i][i] = circ_mat3[k][k]
					circ_mat4[j][j] = circ_mat3[l][l]
#		print circ_mat4	
