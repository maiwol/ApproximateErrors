...
class State_and_Operations(object):
	...
	def __init__(self, circuit, number_qubits, initial_state=None, 
		...
		"""
		"""
		self.prep_gates, self.oper_gates, self.meas_gates = self.classify_gates(
									circuit.gates)
		...
		#				   self.number_qubits, 
		#			           self.number_data_qubits, sym)
		self.operations = self.prep_operations(self.oper_gates)
		self.measurements = self.prep_measurements(self.meas_gates)
...
class Whole_Circuit(object):
	...
	def run_initial_subcircs_tree(self, outcomes, n_proc=2, init_state_data=None, 
					  init_state_anc=None, circuit_list=None, n_qs=None, 
					  prob_limit=1.e-10, sym=None, corr='Shor'):
		'''
